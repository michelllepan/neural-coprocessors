...
from CQL_SAC.networks import Actor, Critic
...
class CQLSAC(nn.Module):
    ...
    def load_critic(self, critic_dir, device):
        path1 = os.path.join(critic_dir, "critic1.pth")
        self.critic1.load_state_dict(torch.load(path1, map_location=torch.device(device)))

        path2 = os.path.join(critic_dir, "critic2.pth")
        self.critic2.load_state_dict(torch.load(path2, map_location=torch.device(device)))
