...
from offpcc.domains.robot_envs.env import EnvObject, ASSETS_PATH
from offpcc.domains.robot_envs.bumps_env import BumpsEnvBase
...
class BumpsNormEnv(BumpsEnvBase):
    ...
    def reset(self):
        ...
        if position_1 - self.bump_diameter * 1.5 > -0.35:
            ranges.append([-0.35, position_1 - self.bump_diameter * 1.5])

        if position_1 + self.bump_diameter * 1.5 < 0.35:
            ranges.append([position_1 + self.bump_diameter * 1.5, 0.35])

        position_2 = self._uniform_ranges(ranges)

        # the idea is that bump_1 should always be the left bump

        if position_2 < position_1:
            self.ori_y_bump1 = position_2
            self.ori_y_bump2 = position_1
        elif position_1 < position_2:
            self.ori_y_bump1 = position_1
            self.ori_y_bump2 = position_2
        else:
            assert False, "(from reset) We must have either position_2 < position_1 or position_1 < position_2."

        # @@@@ determine finger position @@@@@

        # Q: why self.bump_diameter / 2 * 1.5?
        # A: If the finger is initialized self.bump_diameter / 2 from a bump, then it will be touching the bump.
        #    Therefore, we multiple the distance by 1.5 to prevent this happening, while retaining the possibility
        #    that the finger can be close to a bump.

        # y_ur5
        y_ur5_range1 = [-0.4, self.ori_y_bump1 - self.bump_diameter / 2 * 1.5]
