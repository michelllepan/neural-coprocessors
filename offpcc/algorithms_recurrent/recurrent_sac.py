...
from offpcc.basics.utils import get_device, create_target, mean_of_unmasked_elements, polyak_update, save_net, load_net, set_random_seed
...
class RecurrentSAC(RecurrentOffPolicyRLAlgorithm):
    ...
    def __init__(
        ...
        if seed is not None:
            set_random_seed(seed, get_device())
        ...
        # networks
        if michaels:
            self.actor_summarizer = Summarizer(input_dim, hidden_dim, recurrent_type='michaels',cfg=cfg).to(get_device())
        else:
            self.actor_summarizer = Summarizer(input_dim, hidden_dim,recurrent_type='lstm').to(get_device())
    ...
    def simulate(self, observation: np.array) -> np.array:
        with torch.no_grad():
            observation = torch.tensor(observation).unsqueeze(0).unsqueeze(0).float().to(get_device())

            summary, _ = self.actor_summarizer(observation, self.hidden, return_hidden=True)
            action = self.sample_action_from_distribution(summary, deterministic=True, return_log_prob=False)
            return action.view(-1).cpu().numpy()  # view as 1d -> to cpu -> to numpy

    def act_q(self,observation: np.array,batch=False,grad=False):
        with torch.no_grad():
            observation = torch.tensor(observation).unsqueeze(0).unsqueeze(0).float().to(get_device())

            Q1_summary, self.q_hidden = self.Q1_summarizer(observation, self.q_hidden, return_hidden=True)
    ...
    def update_networks(self, b: RecurrentBatch) -> dict:
        ...
        # compute summary
        actor_summary = self.actor_summarizer(b.o).to(get_device())
    ...
    def load_q(self, save_dir: str) -> None:
        load_net(net=self.Q1_summarizer, save_dir=save_dir, save_name="q1_summarizer.pth")
        load_net(net=self.Q1, save_dir=save_dir, save_name="q1.pth")
